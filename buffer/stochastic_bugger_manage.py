...
class SafetyCriticalBuffer(PriorityBuffer):
    ...
    def calculate_priority(self, experience):
        ...
        collision_reward = info['rewards'].get('collision_reward', 0.0)
        on_road_reward = info['rewards'].get('on_road_reward', 0.0)
        ...
        # 1. 碰撞或即将碰撞
        if crashed or collision_reward < -0.5:  # 假设负碰撞奖励表示危险
...
class PerformanceBuffer(PriorityBuffer):
    ...
    def calculate_priority(self, experience):
        ...
        # 从info中获取性能相关指标
        high_speed_reward = info['rewards'].get('high_speed_reward', 0.0)
        on_road_reward = info['rewards'].get('on_road_reward', 0.0)
        right_lane_reward = info['rewards'].get('right_lane_reward', 0.0)
        speed = info.get('speed', 0.0)
...
class DiversityBuffer(PriorityBuffer):
    ...
    def calculate_priority(self, experience):
        ...
        # 创建场景指纹
        scenario_fingerprint = self._create_scenario_fingerprint(info)
    ...
    def _create_scenario_fingerprint(self, info):
        """创建场景指纹，用于识别不同场景"""
        speed = info.get('speed', 0.0)
        on_road = info['rewards'].get('on_road_reward', 0.0) > 0.5
        right_lane = info['rewards'].get('right_lane_reward', 0.0) > 0.5
        ...
        # 碰撞风险
        collision_risk = "high" if info.get('crashed', False) or info['rewards'].get('collision_reward',
                                                                                     0.0) < -0.5 else "low"
...
class CurriculumBuffer(PriorityBuffer):
    ...
    def calculate_priority(self, experience):
        ...
        # 评估样本难度
        difficulty = self._assess_difficulty(info)
    ...
    def _assess_difficulty(self, info):
        """评估样本难度"""
        difficulty = 0.0
...
class StochasticBufferManager:
    def __init__(self):
        # 4个专用Buffers
        self.buffers = [
            SafetyCriticalBuffer(capacity=25000),  # 安全关键样本
            PerformanceBuffer(capacity=30000),  # 性能优化样本
            DiversityBuffer(capacity=25000),  # 多样性样本
            CurriculumBuffer(capacity=20000)  # 课程学习样本
        ]
        self.total_capacity = 100000
        self.min_start_train = 5000  # 最小训练启动样本量
    ...
    def _is_safety_critical(self, experience):
        ...
        # 直接安全违反
        if crashed or collision_reward < -0.5:
            return True
    ...
    def _create_scenario_fingerprint(self, info):
        ...
        return f"{speed_level}_{'on' if on_road else 'off'}_{'right' if right_lane else 'other'}_{'crash' if crashed else 'safe'}"
    ...
    def _calculate_gep_priority(self, experience, buffer_idx):
        ...
        # 课程学习缓冲区：基于难度适应性
            ...
            difficulty = self.buffers[3]._assess_difficulty(info)
    ...
    def sample_batch(self, batch_size):
        ...
        # 基础比例采样，安全关键样本给予更高权重
        weights = [0.4, 0.3, 0.2, 0.1]  # Safety, Performance, Diversity, Curriculum
        samples = []
